...
/**
 *This code takes two command line arguments: the IP address of the fake sender
 * and the IP address of the intended recipient. It creates a raw socket and
 * sets the `IP_HDRINCL` option to include the IP header in the packet. It then
 * constructs both the IP header and the ICMP header and calculates the
 * checksums for each. Finally, it combines the headers into a single buffer and
 * sends the packet using the `sendto` function. To spoof other protocols, you
 * will need to replace the `IPPROTO_ICMP` constant with the appropriate
 * protocol number and replace the `struct icmphdr` type
 */
...
int main(int argc, char *argv[]) {
  ...
  int opt_val = 1;
  if (setsockopt(sockfd, IPPROTO_IP, IP_HDRINCL, &opt_val, sizeof(opt_val)) <
      0) {
  ...
  if (sendto(sockfd, buf, sizeof(ip_hdr) + sizeof(icmp_hdr), 0,
